 ...
 * compiled on Ubuntu linux:
   gcc -DMIKE -pthread -g -o sharedmemory2 sharedmemory2.c
 *
 * you need the -DMIKE to make the second program that doesn't create, initialize, or destroy the shared memory region, only updates
 *
...
#else /* MIKE */
		return(fprintf(stderr,"%5u: Unable to get id for %ld byte region with key 0x%x: errno=%d %s\n",__LINE__,
			sizeof(region[0]),key.key,errno,strerror(errno)));
#endif /* MIKE */
...
#ifdef MIKE
		if ((region->array[0]%2)==1)	/* this is the 2nd process we will only increment odd values to even */
#else /* MIKE */
